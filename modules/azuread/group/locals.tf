...
# This is used to build the group's member list containing member user ids and member group ids...
locals {
  members = concat(data.azuread_users.members.object_ids,data.azuread_groups.members.object_ids)
}
